...
const Age = (props) => {
    ...
    const [counterOn, setCounter] = useState(props.counter)

    const ageCounter = () => {
        const ageData = props.ageData
        const divisor = 1000 * 60 * 60 * 24 * 365; // ms in an year
        const birthTime = new Date(ageData.year, ageData.month, ageData.date, ageData.hour, ageData.minute);
        ...
        if (counterOn) {
            const latestBirthday = new Date(refYear, ageData.month, ageData.date, ageData.hour, ageData.minute)
            const msCount = ((Date.now() - latestBirthday) / divisor).toFixed(12)
    ...
    useEffect(() => {
        ...
        if (counterOn){
            timer = setInterval(() => ageCounter(), 25)
        }
        return () => {
            clearInterval(timer);
        }
